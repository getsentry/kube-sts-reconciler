...
**Context:** rolling out VolumeAttributesClass support for an existing StatefulSet-based broker service
...
| 2 | Who recreates the StatefulSet after orphan-delete? | **Both, behind a flag.** Default mode: recreation happens via the next deploy sync / manual re-apply (controller stays stateless). Optional `--recreate-mode=self`: the controller snapshots the manifest before deleting and re-applies it itself. See §5. |
| 3 | The flow diagram says the deploy system clears the desired-spec annotation; the pseudocode says the controller does. | **The controller clears it** once drift is empty. The deploy system's contract is stamp-and-forget. |
| 4 | Annotation domain: a service-specific domain vs. something service-neutral, given the plan to widen scope beyond the first service. | **`sts-reconciler.sentry.io`** from day one. Service scoping lives in the label selector (`--label-selector`), not the annotation contract. |
...
### 3.1 Desired spec — written by deploy tooling (or a human), cleared by the controller
    ...
    sts-reconciler.sentry.io/desired-pvc-spec: |
      {
        ...
        "claims": {
          "sqlite": {
            "volumeAttributesClassName": "broker-sqlite-vac-i15000t140",
            "storage": "333Gi"
          }
        }
      }
...
|---|---|---|
| `deploy` (default) | After orphan-delete, do nothing. The next deploy sync (or a human `kubectl apply`) recreates the STS. | Controller fully stateless; STS-less window lasts until the next deploy. |
...
Recreate race (`deploy` mode): if the deploy system re-applies while the controller is still in
...
| Images | Distroless, multi-arch; built in CI (GitHub Actions) and pushed to GHCR |
...
- **Primary watch:** StatefulSets, filtered by a label-selector predicate
  (`--label-selector`, empty by default) *and* presence of either reconciler
  annotation. Scoping to specific services (experiment plan step 4) is a flag change.
...
2. **Label-scoped:** deployments should set `--label-selector` to the services that
   need reconciling, widening deliberately.
...
7. **Dry-run:** `--dry-run` flag logs every mutation it *would* make (including the delete)
   and writes status state `DryRun`. The first deployment in a new cluster runs this way.
...
- Alert suggestion (wired into whatever monitoring stack consumes the metrics): page on
  `state="Failed"` gauge > 0 for > 15 min.
...
| Cluster validation | staging cluster, dry-run mode | milestone 2 exit criterion: dry-run output matches the manual runbook steps for a real VAC change |
...
kind e2e; deploy to a staging cluster in dry-run; then first supervised live run with a
manually stamped annotation on a real STS.
*Exit: one real VAC change reconciled end-to-end with a human watching.*

**M3 — Self-recreate mode + deploy-tooling integration (week 3).** `--recreate-mode=self`
(snapshot ConfigMap, re-apply, cleanup). Deploy-tooling change (separate repo/PR): on a
...
- **STS-less window in `deploy` mode.** Between orphan-delete and the next sync, crashed
  ...
  is the window in practice on a staging cluster?
- **Deploy-tooling apply semantics.** Does the deploy system's apply strip unknown live
  annotations (which would erase the controller's status annotation on every deploy)? Must
  be checked during M3; if so, the status annotation moves to the field-manager-safe path
  (server-side apply with a distinct field manager) or onto PVCs.
- **`FileSystemResizePending` semantics per CSI driver.** The "converged-enough" rule in §4
  assumes pod churn finishes filesystem resizes. Validate against the actual drivers in use
  during M2.
